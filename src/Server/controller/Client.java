...
public class Client implements Runnable {
    ...
    private void onReceiveLogin(String received) {
        ...
        if (result.split(";")[0].equals("success")) {
            // set login username
            this.loginPlayer = new PlayerBUS().getByUserName(username);
        }
    ...
    private void onReceiveGetProfile(String received) {
        ...
        // get player data
        Player p = new PlayerBUS().getByUserName(username);
        if (p == null) {
    ...
    private void onReceiveEditProfile(String received) {
        try {
            ...
            // edit profile
            String result = new PlayerBUS().editProfile(loginPlayer.getUserName(), newUserName, name, avatar, yearOfBirth, gender);

            // lưu lại newUserName vào Client nếu cập nhật thành công
            String status = result.split(";")[0];
            if (status.equals("success")) {
                loginPlayer = new PlayerBUS().getByUserName(newUserName);
            }
    ...
    private void onReceiveChangePassword(String received) {
        ...
        // check change pass
        String result = new PlayerBUS().changePassword(loginPlayer.getUserName(), oldPassword, newPassword);
    ...
    private void onReceiveGameEvent(String received) {
        ...
        switch (gameEventType) {
            case MOVE:
                ...
                // check move
                if (caroGame.move(row, column, loginPlayer.getUserName())) {
                    // restart turn timer
                    ...
                    if (winPath != null) {
                        ...
                        // tinh diem
                        winner.addScore(3);
                        winner.addMatchCount(1);
                        winner.addWinCount(1);
                        winner.addcurrentStreak(1);
                        loser.addScore(-2);
                        loser.addMatchCount(1);
                        loser.addLoseCount(1);
                        loser.addcurrentStreak(-999999);
                        bus.update(winner);
                        bus.update(loser);
    ...
    public String getInGameData() {
        ...
        return loginPlayer.getUserName() + ";" + loginPlayer.getNameId() + ";" + loginPlayer.getAvatar();
    }
